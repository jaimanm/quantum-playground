...
const GATE_DESCRIPTIONS: Record<GateType, string> = {
  ...
  M: "Measurement: Collapses qubit to 0 or 1",
};
...
const Gate: React.FC<GateProps> = ({ name }) => {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    // Set custom drag image to include the whole gate box
    if (gateRef.current) {
  ...
  const colors =
    name === "M"
      ? GATE_COLORS.M
      : GATE_COLORS[name as keyof typeof GATE_COLORS] || GATE_COLORS.DEFAULT;
    ...
    <Tooltip.Root delayDuration={300}>
      ...
      <Tooltip.Portal>
        <Tooltip.Content
          ...
          sideOffset={5}
        >
          {GATE_DESCRIPTIONS[name as keyof typeof GATE_DESCRIPTIONS]}
